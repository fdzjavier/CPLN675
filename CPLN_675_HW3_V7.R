...
dc_fish1 <- st_read("~/ArcGIS/Projects/dc/dc_geo5", promote_to_multi = FALSE) #dc_geo5 is uploaded in the github
dc_fishdf <- data.frame(dc_fish1)
fish_geom <- dc_fishdf[, c("CELL", "geometry")]  # fishnet geometry for DC
# renaming "CELL" to fit cleaned DC fishnet
fish_geom1 <- fish_geom %>% rename(cell = CELL)

# joining geometry to cleaned fishnet
cleaned_DCTable_v2 <- read_csv("~/ArcGIS/Projects/dc/cleaned_DCTable_v2.csv")

dc_inundation <- cleaned_DCTable_v2 %>%
  left_join(fish_geom1, by = "cell") # dc_inundation has geometry now
